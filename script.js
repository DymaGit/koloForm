...
function overNabidku() {
    ...
    if (nabid >= cenaZaKolaCelkem) {
        document.querySelector('#schvaleni').value = "Success";
        document.querySelector('#schvaleni').style.color = 'green';
    }
    else {
        document.querySelector('#schvaleni').value = "Failed";
...
function overEmail() {
    ...
    if (mailAddress) {
        document.querySelector('#vyzva').innerHTML = "<span style=color:green><br>Email sent";
    } else {
        document.querySelector('#vyzva').innerHTML = "<span style=color:red><br>Wrong format";
    }
};
